#!/usr/bin/env node
/**
 * BACKTEST: RGRG→G / GRGR→R — Wait 2 Virtual Losses → IMMEDIATE MARTINGALE
 * 
 * Strategy:
 ...
 * - After 2 consecutive virtual losses → Start LIVE Martingale Cycle
 * - Place LIVE bet at ₹10. 
 * - If LIVE bet LOSES → IMMEDIATELY place next step (₹30) on the VERY NEXT signal (no waiting for 2 virtual losses)
 * - Continue immediately until WIN or BUST (4 losses)
 * - On WIN or BUST → reset to virtual tracking, wait for 2 virtual losses again
 * 
...
function runBacktest(periods) {
  let virtualLosses = 0;
  let isLiveCycle = false; // true if we are in an active martingale progression
  let liveStep = 0;
  let activeBet = null;
  ...
  for (let i = 0; i < periods.length; i++) {
    ...
    if (activeBet && periods[i].period === activeBet.period) {
      ...
      if (activeBet.isLive) {
        ...
        if (won) {
          // WIN -> collect profit, reset to virtual
          cycles.push({
          ...
          liveStep = 0;
          isLiveCycle = false;
          virtualLosses = 0; // reset virtual loss counter
        } else {
          // LOSS -> IMMEDIATELY move to next ladder step for the NEXT signal
          liveStep++;
          if (liveStep >= BET_LADDER.length) {
            // BUST -> all steps lost, reset to virtual
            cycles.push({
              trades: [...currentCycleTrades],
              cyclePNL,
              result: 'BUST'
          ...
          // If we didn't bust, we stay in `isLiveCycle = true` and wait for the next signal
        }
      } else {
        ...
        if (won) {
          ...
          if (virtualLosses >= VIRTUAL_LOSS_THRESHOLD) {
            // Reached 2 virtual losses -> Trigger a Live Cycle
            isLiveCycle = true;
            liveStep = 0;
            // Note: we don't reset virtualLosses here so that if we wanted we could track them,
            // but we reset them after the live cycle ends.
          }
        }
      }
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════════════════════');
  console.log('  📊 RGRG→G / GRGR→R — Wait 2 Virtual Losses → IMMEDIATE MARTINGALE');
  console.log('  Entry: Wait for 2 consecutive virtual losses');
  console.log('  Ladder: ₹10 → ₹30 → ₹90 → ₹270 (bet immediately on consecutive signals)');
  console.log('═══════════════════════════════════════════════════════════════════\n');
